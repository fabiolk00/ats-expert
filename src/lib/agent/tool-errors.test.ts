...
describe('tool errors integration', () => {
  ...
  it('derives failures from unknown errors without duplicating code mapping', () => {
    ...
    expect(getHttpStatusForToolError(TOOL_ERROR_CODES.GENERATE_RESUME_LATEST_VERSION_MISSING)).toBe(409)
    expect(getHttpStatusForToolError(TOOL_ERROR_CODES.GENERATE_RESUME_PENDING_GENERATION_MISSING)).toBe(409)
    expect(getHttpStatusForToolError(TOOL_ERROR_CODES.GENERATE_RESUME_PENDING_GENERATION_REUSE_FAILED)).toBe(500)
    expect(getHttpStatusForToolError(TOOL_ERROR_CODES.GENERATE_RESUME_PENDING_GENERATION_CREATE_FAILED)).toBe(500)
    expect(getHttpStatusForToolError(TOOL_ERROR_CODES.GENERATE_RESUME_PENDING_GENERATION_CONSTRAINT_FAILED)).toBe(500)
    expect(getHttpStatusForToolError(TOOL_ERROR_CODES.GENERATE_RESUME_RESERVATION_FAILED)).toBe(500)
    expect(getHttpStatusForToolError(TOOL_ERROR_CODES.GENERATE_RESUME_RENDER_FAILED)).toBe(500)
    expect(getHttpStatusForToolError(TOOL_ERROR_CODES.GENERATE_RESUME_PERSISTENCE_FAILED)).toBe(500)
  })
})
