...
function buildAssessment(overrides: Partial<JobCompatibilityAssessment> = {}): JobCompatibilityAssessment {
  return {
    ...
    criticalGaps: [{
      id: 'gap-unsupported',
      signal: 'Unsupported core requirement',
      kind: 'skill',
      importance: 'core',
      severity: 'critical',
    ...
    lowFit: {
      ...
      thresholdAudit: {
        score: 24,
        minimumScore: 25,
        unsupportedCoreCount: 3,
        totalCoreCount: 4,
        unsupportedCoreRatio: 0.75,
...
describe('low-fit warning gate', () => {
  it('builds assessment-derived low-fit state without running the legacy calculator', () => {
    const gate = buildLowFitWarningGateFromAssessment(buildAssessment())
    ...
    expect(gate.coreRequirementCoverage.topUnsupportedSignalsForDisplay).toEqual([
      'Unsupported core requirement',
    ])
  })
  ...
  it('does not trigger low-fit block when match score is high and core coverage is partial', () => {
    const gate = buildLowFitWarningGate({
      ...
      targetRolePositioning: {
        ...
        permission: 'must_not_claim_target_role',
        reason: 'career_fit_high_risk',
        safeRolePositioning: 'Profissional com experi?ncia em SQL e dashboards.',
      ...
      coreRequirementCoverage: {
        ...
        supported: 4,
        unsupported: 19,
        unsupportedSignals: ['DAX', 'tratamento e integra??o de dados'],
        topUnsupportedSignalsForDisplay: ['DAX', 'Tratamento e integra??o de dados'],
      },
    })
