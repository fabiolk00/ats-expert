...
const inputClass = 'border-[#d8d4ca] bg-white text-[#2c2a25] placeholder:text-[#9c9789] focus-visible:border-[#171717] focus-visible:ring-[#171717]/10'
const cardClass = 'rounded-xl border border-[#e8e5dc] bg-white p-4 shadow-[0_1px_2px_rgba(0,0,0,0.03)]'
const primaryButtonClass = 'bg-[#171717] text-white hover:bg-black'
const secondaryButtonClass = 'border-[#d8d4ca] bg-white text-[#2c2a25] hover:bg-[#f5f5f3]'
    ...
    <AnimatePresence mode="wait" initial={false}>
      <motion.div key={activeTab} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.18, ease: 'easeInOut' }} className="h-full overflow-y-auto px-5 py-4">
        <TabsContent value="summary" forceMount hidden={activeTab !== 'summary'} className={activeTab === 'summary' ? 'mt-0 space-y-4' : 'hidden'}>
          ...
          <Textarea rows={6} value={draft.summary} className={`${inputClass} min-h-[160px] resize-none`} onChange={(event) => update((current) => ({ ...current, summary: event.target.value }))} />
          <p className="text-right text-xs text-[#9c9789]">{draft.summary.length} caracteres</p>
        </TabsContent>
        ...
        <TabsContent value="experience" forceMount hidden={activeTab !== 'experience'} className={activeTab === 'experience' ? 'mt-0 space-y-4' : 'hidden'}>
          <div className="flex items-center justify-between gap-3">
            ...
            <Button type="button" size="sm" className={primaryButtonClass} onClick={() => update((current) => ({ ...current, experience: [...current.experience, { title: '', company: '', location: '', startDate: '', endDate: 'present', bullets: [''] }] }))}><Plus className="h-4 w-4" />Adicionar experiência</Button>
          </div>
          {draft.experience.map((item, index) => (
            <div key={`experience-${index}`} className={`${cardClass} space-y-3`}>
              <div className="flex items-start justify-between gap-3">
                ...
                <Button type="button" variant="ghost" size="icon-sm" className="text-[#9c9789] hover:bg-[#f5f5f3] hover:text-[#171717]" onClick={() => update((current) => ({ ...current, experience: current.experience.filter((_, entryIndex) => entryIndex !== index) }))}><Trash2 className="h-4 w-4" /><span className="sr-only">Remover experiência</span></Button>
              </div>
              <div className="space-y-2">
                <div className="flex items-center justify-between gap-3">
                  <p className="text-xs font-medium uppercase tracking-[0.08em] text-[#5c5647]">Bullets</p>
                  <Button type="button" variant="outline" size="sm" className={secondaryButtonClass} onClick={() => update((current) => ({ ...current, experience: current.experience.map((entry, entryIndex) => entryIndex === index ? { ...entry, bullets: [...entry.bullets, ''] } : entry) }))}><Plus className="h-4 w-4" />Adicionar bullet</Button>
                </div>
                {item.bullets.map((bullet, bulletIndex) => (
                  <div key={`experience-${index}-bullet-${bulletIndex}`} className="flex gap-2">
                    ...
                    <Button type="button" variant="ghost" size="icon-sm" className="shrink-0 text-[#9c9789] hover:bg-[#f5f5f3] hover:text-[#171717]" onClick={() => update((current) => ({ ...current, experience: current.experience.map((entry, entryIndex) => entryIndex === index ? { ...entry, bullets: entry.bullets.filter((_, entryBulletIndex) => entryBulletIndex !== bulletIndex) } : entry) }))}><Trash2 className="h-4 w-4" /><span className="sr-only">Remover bullet</span></Button>
                  </div>
                ))}
              </div>
            </div>
        ...
        <TabsContent value="education" forceMount hidden={activeTab !== 'education'} className={activeTab === 'education' ? 'mt-0 space-y-4' : 'hidden'}>
          <div className="flex items-center justify-between gap-3">
            ...
            <Button type="button" size="sm" className={primaryButtonClass} onClick={() => update((current) => ({ ...current, education: [...current.education, { degree: '', institution: '', year: '', gpa: '' }] }))}><Plus className="h-4 w-4" />Adicionar formação</Button>
          </div>
          {draft.education.map((item, index) => (
            <div key={`education-${index}`} className={`${cardClass} space-y-3`}>
              <div className="flex items-start justify-between gap-3">
                ...
                <Button type="button" variant="ghost" size="icon-sm" className="text-[#9c9789] hover:bg-[#f5f5f3] hover:text-[#171717]" onClick={() => update((current) => ({ ...current, education: current.education.filter((_, entryIndex) => entryIndex !== index) }))}><Trash2 className="h-4 w-4" /><span className="sr-only">Remover formação</span></Button>
              </div>
            </div>
          ))}
        </TabsContent>
        ...
        <TabsContent value="skills" forceMount hidden={activeTab !== 'skills'} className={activeTab === 'skills' ? 'mt-0 space-y-4' : 'hidden'}>
          ...
          <div className="flex gap-2">
            ...
            <Button type="button" className={primaryButtonClass} aria-label="Adicionar skill" onClick={addSkill}>Adicionar</Button>
          </div>
        </TabsContent>
        ...
        <TabsContent value="certifications" forceMount hidden={activeTab !== 'certifications'} className={activeTab === 'certifications' ? 'mt-0 space-y-4' : 'hidden'}>
          <div className="flex items-center justify-between gap-3">
            ...
            <Button type="button" size="sm" className={primaryButtonClass} onClick={() => update((current) => ({ ...current, certifications: [...(current.certifications ?? []), { name: '', issuer: '', year: '' }] }))}><Plus className="h-4 w-4" />Adicionar certificação</Button>
          </div>
          {(draft.certifications ?? []).map((item, index) => (
            <div key={`certification-${index}`} className={`${cardClass} space-y-3`}>
              <div className="flex items-start justify-between gap-3">
                ...
                <Button type="button" variant="ghost" size="icon-sm" className="text-[#9c9789] hover:bg-[#f5f5f3] hover:text-[#171717]" onClick={() => update((current) => ({ ...current, certifications: (current.certifications ?? []).filter((_, entryIndex) => entryIndex !== index) }))}><Trash2 className="h-4 w-4" /><span className="sr-only">Remover certificação</span></Button>
              </div>
            </div>
          ))}
        </TabsContent>
      ...
      <DialogContent showCloseButton={!isSaving} className="flex max-h-[85vh] max-w-[92vw] flex-col gap-0 overflow-hidden border-[#e8e5dc] bg-white p-0 text-[#2c2a25] sm:max-w-4xl">
        ...
        <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as Tab)} className="flex min-h-0 flex-1 flex-col overflow-hidden">
          <TabsList className="h-auto flex-wrap justify-start gap-1 rounded-none border-b border-[#e8e5dc] bg-transparent px-4 py-3 text-[#9c9789]">
            <TabsTrigger value="summary" className="data-[state=active]:bg-[#f5f5f3] data-[state=active]:text-[#171717]">Resumo</TabsTrigger>
            <TabsTrigger value="experience" className="data-[state=active]:bg-[#f5f5f3] data-[state=active]:text-[#171717]">Experiência</TabsTrigger>
            <TabsTrigger value="education" className="data-[state=active]:bg-[#f5f5f3] data-[state=active]:text-[#171717]">Educação</TabsTrigger>
            <TabsTrigger value="skills" className="data-[state=active]:bg-[#f5f5f3] data-[state=active]:text-[#171717]">Skills</TabsTrigger>
            <TabsTrigger value="contact" className="data-[state=active]:bg-[#f5f5f3] data-[state=active]:text-[#171717]">Contato</TabsTrigger>
            <TabsTrigger value="certifications" className="data-[state=active]:bg-[#f5f5f3] data-[state=active]:text-[#171717]">Certificações</TabsTrigger>
          </TabsList>
          <div className="min-h-0 flex-1 overflow-hidden">
            ...
            {!isLoading && error ? <div className="flex h-full flex-col items-center justify-center gap-3 px-5 py-10 text-center"><p className="text-sm text-[#9c9789]">{error}</p><Button type="button" variant="outline" className={secondaryButtonClass} onClick={() => void refetch()}>Tentar novamente</Button></div> : null}
            {body}
          </div>
        </Tabs>
        <DialogFooter className="items-center justify-between border-t border-[#e8e5dc] px-5 py-3">
          ...
          <div className="flex gap-2">
            <Button type="button" variant="outline" className={secondaryButtonClass} disabled={isSaving} onClick={close}>Cancelar</Button>
            <Button type="button" disabled={isSaving || !draft || isLoading || Boolean(error)} className={primaryButtonClass} onClick={() => void save()}>
