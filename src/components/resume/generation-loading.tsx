...
export function GenerationLoading({
  ...
  useEffect(() => {
    ...
    // Simular progresso com velocidade variavel
    const getIncrement = () => {
  ...
  useEffect(() => {
    if (!isLoading) {
      // Completar a barra quando terminar
      setProgress(100)
      const timer = setTimeout(() => {
        setIsVisible(false)
        onComplete?.()
  ...
  // Trocar mensagens baseado no progresso
  useEffect(() => {
  ...
  return (
    <div
      className={cn(
        "fixed inset-0 z-50 flex items-center justify-center bg-white transition-opacity duration-500 dark:bg-zinc-950",
