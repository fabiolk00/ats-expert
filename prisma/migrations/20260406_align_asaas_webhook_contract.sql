...
BEGIN
  ...
  IF p_event_type NOT IN ('SUBSCRIPTION_UPDATED', 'SUBSCRIPTION_CANCELED') THEN
    RAISE EXCEPTION 'Unsupported billing subscription metadata event type: %', p_event_type;
  END IF;

  IF p_asaas_subscription_id IS NULL THEN
    RAISE EXCEPTION 'asaas_subscription_id is required for % events.', p_event_type;
  END IF;
