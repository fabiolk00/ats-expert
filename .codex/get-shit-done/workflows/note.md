...
- **Global scope**: `C:/CurrIA/.codex/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/CurrIA/.codex/notes/`)
...
2. Glob `C:/CurrIA/.codex/notes/*.md` (if directory exists) — global notes
...
2. **No `.planning/`**: Falls back to global `C:/CurrIA/.codex/notes/` — works in any directory
