...
metadata:
  short-description: "Query, inspect, or refresh codebase intelligence files in .planning/intel/"
---

<codex_skill_adapter>
## A. Skill Invocation
- This skill is invoked by mentioning `$gsd-intel`.
- Treat all user text after `$gsd-intel` as `{{GSD_ARGS}}`.
...
Intel system is disabled. To activate:
  ...
  node C:/CurrIA/.codex/get-shit-done/bin/gsd-tools.cjs config-set intel.enabled true
...
```bash
node C:/CurrIA/.codex/get-shit-done/bin/gsd-tools.cjs intel query <term>
```
...
```bash
node C:/CurrIA/.codex/get-shit-done/bin/gsd-tools.cjs intel status
```
...
```bash
node C:/CurrIA/.codex/get-shit-done/bin/gsd-tools.cjs intel diff
```
...
Project root: ${CWD}
gsd-tools path: C:/CurrIA/.codex/get-shit-done/bin/gsd-tools.cjs
